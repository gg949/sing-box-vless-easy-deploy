...
create_config() {
    ...
    # 动态构建 JSON，只加入选中的协议
    {
    echo '{"log":{"level":"warn","timestamp":true},"inbounds":['
    first=true
    if $INSTALL_SS; then
        $first || echo ","
        first=false
        echo "{\"type\":\"shadowsocks\",\"listen\":\"::\",\"listen_port\":$PORT_SS,\"method\":\"2022-blake3-aes-128-gcm\",\"password\":\"$PSK_SS\",\"tag\":\"ss-in\"}"
    fi
    if $INSTALL_HY2; then
        $first || echo ","
        first=false
        echo "{\"type\":\"hysteria2\",\"tag\":\"hy2-in\",\"listen\":\"::\",\"listen_port\":$PORT_HY2,\"users\":[{\"password\":\"$PSK_HY2\"}],\"tls\":{\"enabled\":true,\"alpn\":[\"h3\"],\"certificate_path\":\"/etc/sing-box/certs/fullchain.pem\",\"key_path\":\"/etc/sing-box/certs/privkey.pem\"}}"
    fi
    if $INSTALL_REALITY; then
        $first || echo ","
        first=false
        echo "{\"type\":\"vless\",\"tag\":\"vless-in\",\"listen\":\"::\",\"listen_port\":$PORT_REALITY,\"tcp_fast_open\":true,\"users\":[{\"uuid\":\"$UUID\",\"flow\":\"xtls-rprx-vision\"}],\"tls\":{\"enabled\":true,\"server_name\":\"addons.mozilla.org\",\"reality\":{\"enabled\":true,\"handshake\":{\"server\":\"addons.mozilla.org\",\"server_port\":443},\"private_key\":\"$REALITY_PK\",\"short_id\":[\"$REALITY_SID\"]}}}"
    fi
    if $INSTALL_VLESS_WS; then
        $first || echo ","
        first=false
        echo "{\"type\":\"vless\",\"tag\":\"vless-ws-in\",\"listen\":\"::\",\"listen_port\":$PORT_VLESS_WS,\"tcp_fast_open\":true,\"users\":[{\"uuid\":\"$UUID_VLESS_WS\"}],\"transport\":{\"type\":\"ws\",\"path\":\"/\"}}"
    fi
    echo '],"outbounds":[{"type":"direct","tag":"direct-out"}]}'
    } > "$CONFIG_PATH"
