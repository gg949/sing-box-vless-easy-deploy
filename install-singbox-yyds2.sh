...
if $INSTALL_ARGO; then
    ...
    echo "Argo 隧道搭建"
    echo "=========================="
    echo "1) 临时隧道（免配置，一键启动）"
    echo "2) 固定隧道（需 Token + 域名）"
    echo "=========================="
    ...
    if [ "$argo_choice" = "1" ]; then
        # 临时隧道
        install_cloudflared
        if [ -f /tmp/cloudflared-temp.pid ] && kill -0 "$(cat /tmp/cloudflared-temp.pid)" 2>/dev/null; then
            kill "$(cat /tmp/cloudflared-temp.pid)" 2>/dev/null || true
            rm -f /tmp/cloudflared-temp.pid
        fi
        info "启动 Argo 临时隧道，转发到本地 $PORT_VLESS_WS 端口..."
        nohup cloudflared tunnel --url "http://localhost:${PORT_VLESS_WS}" > /tmp/cloudflared-temp.log 2>&1 &
        echo $! > /tmp/cloudflared-temp.pid
        argo_url=""
        for i in 1 2 3 4 5 6 7; do
            sleep 3
            argo_url=$(grep -oP 'https://[a-zA-Z0-9.-]+\.trycloudflare\.com' /tmp/cloudflared-temp.log 2>/dev/null | head -1 || true)
            [ -n "$argo_url" ] && break
            grep -qi "error\|failed\|refused" /tmp/cloudflared-temp.log 2>/dev/null && break
        done
        if [ -n "$argo_url" ]; then
            echo ""
            echo "=== Argo 临时隧道节点 ==="
            echo "vless://${UUID_VLESS_WS}@${argo_prefer_host}:${argo_prefer_port}?encryption=none&type=ws&path=%2F&security=tls&sni=${argo_prefer_host}#argo-temp"
            echo ""
            echo "隧道地址: $argo_url"
            echo "（重启后地址会变，需重新搭建）"
        else
        ...
        echo "先在 Cloudflare Zero Trust 创建隧道:"
...
UNIT
        ...
        if systemctl is-active cloudflared-tunnel >/dev/null 2>&1; then
            echo ""
            echo "=== Argo 固定隧道节点 ==="
            echo "vless://${UUID_VLESS_WS}@${argo_prefer_host}:${argo_prefer_port}?encryption=none&type=ws&path=%2F&security=tls&sni=${argo_prefer_host}#argo"
            echo ""
            echo "域名: $tunnel_domain → localhost:$PORT_VLESS_WS"
            echo "※ 还需在 Cloudflare Zero Trust 中配置 Public Hostname"
        else
            err "隧道启动失败: journalctl -u cloudflared-tunnel -n 30 --no-pager"
        fi
    else
        warn "无效选项，跳过 Argo 隧道搭建"
    fi
    echo ""
fi
...
action_argo_remove() {
    ...
    rm -f /tmp/cloudflared-temp.pid /tmp/cloudflared-temp.log
    info "Argo 隧道已清理"
}
...
MENU
    ...
    case "${opt:-}" in
        ...
        18) action_reset_vless_ws || true ;;
                19) action_uninstall; exit 0 ;;
        20) action_remove_ss || true ;;
        21) action_remove_hy2 || true ;;
        22) action_remove_reality || true ;;
        23) action_remove_vless_ws || true ;;
        0) exit 0 ;;
        *) warn "无效选项" ;;
    esac
