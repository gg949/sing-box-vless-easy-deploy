...
echo "  4) VLESS WS (CDN 回源)"
echo "  5) Argo 隧道 (cloudflared)"
echo "  0) 全部安装"
echo "=========================="
echo "示例：输入 1,2,3 安装前三个，或输入 0 全部安装"
read -p "请选择: " proto_choice
...
if [ "$proto_choice" = "0" ]; then
    ...
    for sel in $(echo "$proto_choice" | tr ',' ' '); do
        case "$sel" in
            1) INSTALL_SS=true ;;
            2) INSTALL_HY2=true ;;
            3) INSTALL_REALITY=true ;;
            4) INSTALL_VLESS_WS=true ;;
            5) INSTALL_ARGO=true ;;
        esac
    done
fi
...
echo ""
echo "已选协议:"
$INSTALL_SS && echo "  ✅ Shadowsocks" || echo "  ❌ Shadowsocks"
$INSTALL_HY2 && echo "  ✅ Hysteria2" || echo "  ❌ Hysteria2"
$INSTALL_REALITY && echo "  ✅ VLESS Reality" || echo "  ❌ VLESS Reality"
    if $INSTALL_VLESS_WS; then
        if [ "${ARGO_AUTO_VLESS:-false}" = "true" ]; then
            echo "  ✅ VLESS WS (Argo 专用)"
        else
            echo "  ✅ VLESS WS (CDN回源)"
        fi
    else
...
$INSTALL_ARGO && echo "  ✅ Argo 隧道" || echo "  ❌ Argo 隧道"
echo ""

# 如果只选 Argo，自动添加 VLESS WS 作为 Argo 的本地节点
if $INSTALL_ARGO && ! $INSTALL_SS && ! $INSTALL_HY2 && ! $INSTALL_REALITY && ! $INSTALL_VLESS_WS; then
    INSTALL_VLESS_WS=true
    ARGO_AUTO_VLESS=true
    info "Argo 需要 VLESS WS 作为本地节点，已自动添加"
else
    ARGO_AUTO_VLESS=false
fi

# 是否需要安装 sing-box
if $INSTALL_SS || $INSTALL_HY2 || $INSTALL_REALITY || $INSTALL_VLESS_WS; then
    SKIP_SINGBOX=false
else
    SKIP_SINGBOX=true
fi
...
# 初始化变量（防止 -u 模式下引用未定义变量）"
PORT_SS=""; PSK_SS=""; SS_METHOD="2022-blake3-aes-128-gcm"
PORT_HY2=""; PSK_HY2=""
PORT_REALITY=""; UUID=""; REALITY_PK=""; REALITY_PUB=""; REALITY_SID=""
PORT_VLESS_WS=""; UUID_VLESS_WS=""
...
get_config() {
    ...
    if $INSTALL_VLESS_WS; then
        info "=== 配置 VLESS WS (CDN 回源) ==="
        if [ -n "${SINGBOX_PORT_VLESS_WS:-}" ]; then
            PORT_VLESS_WS="$SINGBOX_PORT_VLESS_WS"
            info "使用环境变量端口 (VLESS WS): $PORT_VLESS_WS"
        else
            read -p "请输入 VLESS WS 端口（留空则随机 10000-60000）: " USER_PORT_VLESS_WS
            if [ -z "$USER_PORT_VLESS_WS" ]; then
                PORT_VLESS_WS=$(shuf -i 10000-60000 -n 1 2>/dev/null || echo $((RANDOM % 50001 + 10000)))
                info "使用随机端口 (VLESS WS): $PORT_VLESS_WS"
            else
                PORT_VLESS_WS="$USER_PORT_VLESS_WS"
            fi
        fi

        UUID_VLESS_WS=$(cat /proc/sys/kernel/random/uuid)
        info "已生成 VLESS WS UUID: $UUID_VLESS_WS"
    fi
}
...
# 安装 sing-box（如果选了需要 sing-box 的协议）
...
create_config() {
    ...
    entries=""
    if $INSTALL_SS; then
        entries="$entries{\"type\":\"shadowsocks\",\"listen\":\"::\",\"listen_port\":$PORT_SS,\"method\":\"2022-blake3-aes-128-gcm\",\"password\":\"$PSK_SS\",\"tag\":\"ss-in\"},"
    fi
    if $INSTALL_HY2; then
        entries="$entries{\"type\":\"hysteria2\",\"tag\":\"hy2-in\",\"listen\":\"::\",\"listen_port\":$PORT_HY2,\"users\":[{\"password\":\"$PSK_HY2\"}],\"tls\":{\"enabled\":true,\"alpn\":[\"h3\"],\"certificate_path\":\"/etc/sing-box/certs/fullchain.pem\",\"key_path\":\"/etc/sing-box/certs/privkey.pem\"}},"
    fi
    if $INSTALL_REALITY; then
        entries="$entries{\"type\":\"vless\",\"tag\":\"vless-in\",\"listen\":\"::\",\"listen_port\":$PORT_REALITY,\"tcp_fast_open\":true,\"users\":[{\"uuid\":\"$UUID\",\"flow\":\"xtls-rprx-vision\"}],\"tls\":{\"enabled\":true,\"server_name\":\"addons.mozilla.org\",\"reality\":{\"enabled\":true,\"handshake\":{\"server\":\"addons.mozilla.org\",\"server_port\":443},\"private_key\":\"$REALITY_PK\",\"short_id\":[\"$REALITY_SID\"]}}},"
    fi
    if $INSTALL_VLESS_WS; then
        entries="$entries{\"type\":\"vless\",\"tag\":\"vless-ws-in\",\"listen\":\"::\",\"listen_port\":$PORT_VLESS_WS,\"tcp_fast_open\":true,\"users\":[{\"uuid\":\"$UUID_VLESS_WS\"}],\"transport\":{\"type\":\"ws\",\"path\":\"/\"}},"
    fi
    # 去掉最后一个逗号
    entries="${entries%,}"
    ...
    # 写出完整配置
    cat > "$CONFIG_PATH" <<EOF
{"log":{"level":"warn","timestamp":true},"inbounds":[$entries],"outbounds":[{"type":"direct","tag":"direct-out"}]}
EOF
    info "配置已写入: $(echo "$entries" | grep -o '"tag":"[^"]*"' | wc -l) 个入站"

    sing-box check -c "$CONFIG_PATH" 2>&1 \
       && info "配置文件验证通过" \
       || warn "配置文件验证失败，继续执行"
...
if ! $SKIP_SINGBOX; then
info "📋 配置信息："
if $INSTALL_SS; then echo "   SS 端口: $PORT_SS | 密码: $PSK_SS"; fi
if $INSTALL_HY2; then echo "   HY2 端口: $PORT_HY2 | 密码: $PSK_HY2"; fi
if $INSTALL_REALITY; then echo "   Reality 端口: $PORT_REALITY | UUID: $UUID"; fi
if $INSTALL_VLESS_WS; then echo "   VLESS WS 端口: $PORT_VLESS_WS | UUID: $UUID_VLESS_WS"; fi
echo "   服务器: $PUB_IP"
fi
echo ""
info "📂 文件位置："
...
read_config_fields() {
    ...
    # VLESS WS (CDN 回源)
    VLESS_WS_PORT=$(jq -r '.inbounds[] | select(.tag=="vless-ws-in") | .listen_port // empty' "$CONFIG_PATH" | head -n1 || true)
    VLESS_WS_UUID=$(jq -r '.inbounds[] | select(.tag=="vless-ws-in") | .users[0].uuid // empty' "$CONFIG_PATH" | head -n1 || true)
...
generate_and_save_uris() {
...

    {
        ...
        if [ -n "$VLESS_WS_PORT" ] && [ -n "$VLESS_WS_UUID" ]; then
            vless_ws_uri="vless://${VLESS_WS_UUID}@${PUBLIC_IP}:${VLESS_WS_PORT}?encryption=none&type=ws&path=%2F#vless-ws${node_suffix}"
            echo ""
            echo "=== VLESS WS (CDN 回源) ==="
            echo "$vless_ws_uri"
            echo ""
            echo "※ 回源方式：将域名指向本机 IP，CF 开启代理，回源端口设为 $VLESS_WS_PORT"
            echo "※ 客户端 address 填你的域名，端口 443，开启 TLS，host 填你的域名"
        fi
        if [ -n "$relay_uri" ]; then
            echo ""
...
# 查看 VLESS WS (CDN 回源)
action_view_vless_ws() {
    read_config_fields || return 1
    if [ -z "$VLESS_WS_PORT" ] || [ -z "$VLESS_WS_UUID" ]; then
        err "未找到 VLESS WS 配置，请先安装或编辑配置文件"
        return 1
    fi
    PUBLIC_IP=$(get_public_ip || true)
    [ -z "$PUBLIC_IP" ] && PUBLIC_IP="YOUR_SERVER_IP"
    ...
    echo ""
    echo "=== VLESS WS (CDN 回源) ==="
    echo "vless://${VLESS_WS_UUID}@${PUBLIC_IP}:${VLESS_WS_PORT}?encryption=none&type=ws&path=%2F#vless-ws${node_suffix}"
    echo ""
    echo "※ 使用方式（CF 回源）："
    echo "  1. 在 Cloudflare 添加域名的 A 记录指向本机 IP"
    echo "  2. 开启橙色云朵（Proxy）"
    echo "  3. 在 CF 规则中设置回源端口为 $VLESS_WS_PORT"
    echo "  4. 客户端配置：address=你的域名, port=443, TLS 开启, host=你的域名, path=/"
    echo ""
}
...
action_reset_vless_ws() {
    read_config_fields || return 1
    if [ -z "$VLESS_WS_PORT" ]; then
        err "当前配置中未找到 VLESS WS 入站"
        return 1
    fi

    read -p "输入新的 VLESS WS 端口（回车保持 $VLESS_WS_PORT）: " new_ws_port
    [ -z "$new_ws_port" ] && new_ws_port="$VLESS_WS_PORT"
    if ! valid_port "$new_ws_port"; then
        err "端口无效: $new_ws_port"
        return 1
    fi

    read -p "输入新的 VLESS WS UUID（回车随机生成）: " new_ws_uuid
    [ -z "$new_ws_uuid" ] && new_ws_uuid=$(cat /proc/sys/kernel/random/uuid)

    info "正在停止服务..."
    service_stop || warn "停止服务失败"
    cp "$CONFIG_PATH" "${CONFIG_PATH}.bak"

    jq --argjson port "$new_ws_port" --arg uuid "$new_ws_uuid" '
    .inbounds |= map(
        if .tag=="vless-ws-in" then
            .listen_port = $port |
            (.users[0].uuid) = $uuid
        else .
        end
    ...
    info "已更新 VLESS WS 端口($new_ws_port)与 UUID，正在启动服务..."
    service_start || warn "启动服务失败"
    sleep 1
    generate_and_save_uris || warn "生成 URI 失败"
}
...
# ============================================================
# Argo Tunnel (Cloudflare Tunnel) 支持
# ============================================================
ARGO_SERVICE_NAME="cloudflared"
ARGO_TEMP_PID_FILE="/tmp/cloudflared-temp.pid"
ARGO_FIXED_CONFIG_FILE="/etc/sing-box/argo-config.yml"
...
install_cloudflared() {
    ...
    case "$OS" in
        alpine)
            apk add --no-cache cloudflared 2>/dev/null || {
                err "Alpine 请手动安装 cloudflared"
                return 1
            }
            ;;
        debian)
            ARCH=$(dpkg --print-architecture 2>/dev/null || echo "amd64")
            curl -sL "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-${ARCH}.deb" -o /tmp/cloudflared.deb
            dpkg -i /tmp/cloudflared.deb 2>/dev/null || { apt-get install -f -y && dpkg -i /tmp/cloudflared.deb; }
        ...
        redhat)
            ...
            rpm -i /tmp/cloudflared.rpm 2>/dev/null || yum localinstall -y /tmp/cloudflared.rpm
            rm -f /tmp/cloudflared.rpm
            ;;
    esac
    if command -v cloudflared >/dev/null 2>&1; then
        info "cloudflared 安装成功"
    else
        err "cloudflared 安装失败，请手动安装"
        return 1
    fi
}
...
# Argo 临时隧道
action_argo_temp() {
    read_config_fields 2>/dev/null || true
    local_ws_port="${VLESS_WS_PORT:-}"
    if [ -z "$local_ws_port" ]; then
        read -p "请输入要转发的本地端口: " local_ws_port
        if [ -z "$local_ws_port" ] || ! valid_port "$local_ws_port"; then
            err "端口无效"
            return 1
        fi
    fi
    ...
    echo ""
    echo "=== 节点优选配置（客户端连接用）==="
    read -p "优选域名或 IP（回车默认 www.shopify.com）: " argo_prefer_host
    [ -z "$argo_prefer_host" ] && argo_prefer_host="www.shopify.com"
    read -p "优选端口（回车默认 443）: " argo_prefer_port
    [ -z "$argo_prefer_port" ] && argo_prefer_port="443"
    if ! valid_port "$argo_prefer_port"; then
        err "端口无效: $argo_prefer_port"
        return 1
    fi
    # 保存优选配置供后续使用
    echo "ARGO_PREFER_HOST=$argo_prefer_host" > /etc/sing-box/.argo_prefer
    echo "ARGO_PREFER_PORT=$argo_prefer_port" >> /etc/sing-box/.argo_prefer
    install_cloudflared || return 1
    if [ -f "$ARGO_TEMP_PID_FILE" ] && kill -0 "$(cat "$ARGO_TEMP_PID_FILE")" 2>/dev/null; then
        kill "$(cat "$ARGO_TEMP_PID_FILE")" 2>/dev/null || true
        rm -f "$ARGO_TEMP_PID_FILE"
    fi
    info "启动 Argo 临时隧道，转发到本地 $local_ws_port 端口..."
    nohup cloudflared tunnel --url "http://localhost:${local_ws_port}" > /tmp/cloudflared-temp.log 2>&1 &
    echo $! > "$ARGO_TEMP_PID_FILE"
    # 等待隧道连接，最多等 20 秒
    argo_url=""
    for i in 1 2 3 4 5 6 7; do
        sleep 3
        argo_url=$(grep -oP 'https://[a-zA-Z0-9.-]+\.trycloudflare\.com' /tmp/cloudflared-temp.log 2>/dev/null | head -1 || true)
        if [ -n "$argo_url" ]; then
            break
        fi
        # 检查是否报错
        if grep -qi "error\|failed\|refused" /tmp/cloudflared-temp.log 2>/dev/null; then
            break
        fi
    done
    if [ -n "$argo_url" ]; then
        ...
        read_config_fields 2>/dev/null || true
        # 读取优选配置
        [ -f /etc/sing-box/.argo_prefer ] && source /etc/sing-box/.argo_prefer
        argo_prefer_host="${ARGO_PREFER_HOST:-www.shopify.com}"
        argo_prefer_port="${ARGO_PREFER_PORT:-443}"
        if [ -n "${VLESS_WS_UUID:-}" ]; then
            node_suffix=$(cat /root/node_names.txt 2>/dev/null || true)
            echo ""
            echo "=== VLESS+WS over Argo 节点 ==="
            echo "vless://${VLESS_WS_UUID}@${argo_prefer_host}:${argo_prefer_port}?encryption=none&type=ws&path=%2F&security=tls&sni=${argo_prefer_host}#argo-temp${node_suffix}"
        fi
        echo ""
        echo "（重启后隧道地址会变，可通过选项 22 重新获取）"
    else
        warn "隧道连接未就绪，日志内容:"
        tail -10 /tmp/cloudflared-temp.log 2>/dev/null || true
        echo ""
        info "可以稍后手动查看日志: tail -f /tmp/cloudflared-temp.log"
        info "或重新运行此选项重试"
    fi
}

# Argo 固定隧道 — 只需 token + 域名
action_argo_fixed() {
    read_config_fields 2>/dev/null || true
    local_ws_port="${VLESS_WS_PORT:-}"
    if [ -z "$local_ws_port" ]; then
    ...
    echo ""
    echo "=== Argo 固定隧道（Token 方式）==="
    echo "先在 Cloudflare Zero Trust 创建隧道并获取 token："
    echo "  https://dash.cloudflare.com/ → Zero Trust → Networks → Tunnels → 创建隧道"
    echo ""

    read -p "粘贴 Tunnel Token（eyJ...）: " tunnel_token
    [ -z "$tunnel_token" ] && { err "Token 不能为空"; return 1; }

    read -p "输入你的域名（如 node.example.com）: " tunnel_domain
    [ -z "$tunnel_domain" ] && { err "域名不能为空"; return 1; }
    ...
    # 创建 systemd 服务，直接带上 token
    cat > /etc/systemd/system/cloudflared-tunnel.service <<UNIT
[Unit]
Description=Cloudflare Tunnel (${tunnel_domain})
...
UNIT
    ...
    if systemctl is-active cloudflared-tunnel >/dev/null 2>&1; then
        info "✅ Argo 固定隧道已启动"
        info "域名: $tunnel_domain → localhost:$local_ws_port"
        echo ""
        [ -f /etc/sing-box/.argo_prefer ] && source /etc/sing-box/.argo_prefer
        argo_prefer_host="${ARGO_PREFER_HOST:-www.shopify.com}"
        argo_prefer_port="${ARGO_PREFER_PORT:-443}"
        node_suffix=$(cat /root/node_names.txt 2>/dev/null || true)
        echo "=== Argo 固定隧道节点 ==="
        echo "vless://${VLESS_WS_UUID}@${argo_prefer_host}:${argo_prefer_port}?encryption=none&type=ws&path=%2F&security=tls&sni=${argo_prefer_host}#argo${node_suffix}"
        echo ""
        echo "※ 还需在 Cloudflare Zero Trust 中配置 Public Hostname:"
        echo "   $tunnel_domain → localhost:$local_ws_port"
    else
        err "隧道启动失败，查看日志: journalctl -u cloudflared-tunnel -n 30 --no-pager"
    fi
}
...
action_argo_status() {
    ...
    if [ -f "$ARGO_TEMP_PID_FILE" ] && kill -0 "$(cat "$ARGO_TEMP_PID_FILE")" 2>/dev/null; then
        argo_url=$(grep -oP 'https://[a-zA-Z0-9.-]+\.trycloudflare\.com' /tmp/cloudflared-temp.log 2>/dev/null | head -1 || true)
        echo "临时隧道: 运行中 (PID $(cat "$ARGO_TEMP_PID_FILE"))"
        if [ -n "$argo_url" ]; then
            echo "隧道地址: $argo_url"
        else
            echo "隧道地址: 正在连接中，稍后查看日志"
        fi
        echo "日志: tail -f /tmp/cloudflared-temp.log"
    else
        echo "临时隧道: 未运行"
    fi
    if command -v cloudflared >/dev/null 2>&1; then
        systemctl is-active cloudflared-tunnel 2>/dev/null && echo "固定隧道: 运行中" || echo "固定隧道: 未运行"
    else
        echo "cloudflared: 未安装"
    fi
}

# 查看 Argo 临时隧道链接（从日志中提取）
action_argo_show_url() {
    if [ ! -f /tmp/cloudflared-temp.log ]; then
        err "未找到 Argo 临时隧道日志，请先启动临时隧道"
        return 1
    fi
    argo_url=$(grep -oP 'https://[a-zA-Z0-9.-]+\.trycloudflare\.com' /tmp/cloudflared-temp.log 2>/dev/null | head -1 || true)
    if [ -z "$argo_url" ]; then
        warn "隧道地址尚未获取到，日志内容:"
        tail -5 /tmp/cloudflared-temp.log 2>/dev/null || true
        echo ""
        info "隧道可能还在连接中，请稍后重试"
        info "或查看日志: tail -f /tmp/cloudflared-temp.log"
        return 1
    fi
    echo ""
    echo "=== Argo 临时隧道 ==="
    ...
    if [ -n "${VLESS_WS_UUID:-}" ]; then
        ...
        echo "隧道已就绪，可以配合任意本地服务使用"
        echo "如需 VLESS+WS 节点，请先安装 VLESS WS 协议"
    fi
}

# 停止 Argo 临时隧道
action_argo_stop_temp() {
    if [ -f "$ARGO_TEMP_PID_FILE" ]; then
        kill "$(cat "$ARGO_TEMP_PID_FILE")" 2>/dev/null && info "临时隧道已停止" || warn "停止失败"
        rm -f "$ARGO_TEMP_PID_FILE"
    else
        info "没有运行的临时隧道"
    fi
...
# 卸载 Argo
action_argo_remove() {
    action_argo_stop_temp 2>/dev/null || true
...
remove_inbound_by_tag() {
    local tag="$1"
    local name="$2"
    if ! jq -e ".inbounds[] | select(.tag=="$tag")" "$CONFIG_PATH" >/dev/null 2>&1; then
        info "未找到 $name 入站，无需移除"
        return 1
    fi
    echo ""
    read -p "确认移除 $name 节点？(y/N): " confirm
    [[ ! "$confirm" =~ ^[Yy]$ ]] && { info "已取消"; return 1; }
    info "正在停止服务..."
    service_stop || warn "停止服务失败"
    cp "$CONFIG_PATH" "${CONFIG_PATH}.bak"
    jq "del(.inbounds[] | select(.tag=="$tag"))" "$CONFIG_PATH" > "${CONFIG_PATH}.tmp" && mv "${CONFIG_PATH}.tmp" "$CONFIG_PATH"
    info "已移除 $name 入站，正在启动服务..."
    service_start || warn "启动服务失败"
    sleep 1
    generate_and_save_uris || warn "生成 URI 失败"
    info "✅ $name 已移除"
...
action_remove_ss()     { remove_inbound_by_tag "ss-in"         "Shadowsocks"; }
action_remove_hy2()    { remove_inbound_by_tag "hy2-in"        "Hysteria2"; }
action_remove_reality(){ remove_inbound_by_tag "vless-in"      "VLESS Reality"; }
action_remove_vless_ws(){ remove_inbound_by_tag "vless-ws-in"  "VLESS WS"; }
...
action_uninstall() {
    ...
    action_argo_remove 2>/dev/null || true
...
16) 生成旧版线路机脚本 (SS出站)
17) 查看 VLESS WS (CDN 回源)
18) 重置 VLESS WS 端口/UUID
19) Argo 临时隧道
20) Argo 固定隧道
21) Argo 隧道状态
22) 查看 Argo 临时隧道链接
...
MENU
    ...
    case "${opt:-}" in
        ...
        17) action_view_vless_ws || true ;;
        18) action_reset_vless_ws || true ;;
        19) action_argo_temp || true ;;
        20) action_argo_fixed || true ;;
        21) action_argo_status || true ;;
        22) action_argo_show_url || true ;;
        23) action_argo_stop_temp || true ;;
        24) action_uninstall; exit 0 ;;
        25) action_remove_ss || true ;;
        26) action_remove_hy2 || true ;;
        27) action_remove_reality || true ;;
        28) action_remove_vless_ws || true ;;
        0) exit 0 ;;
        *) warn "无效选项" ;;
    esac
