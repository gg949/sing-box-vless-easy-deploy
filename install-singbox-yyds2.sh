...
create_config() {
    ...
    # 用 jq 动态构建配置，只加入选中的协议
    CONFIG_JSON='{"log":{"level":"warn","timestamp":true},"inbounds":[],"outbounds":[{"type":"direct","tag":"direct-out"}]}'

    if $INSTALL_SS; then
        CONFIG_JSON=$(echo "$CONFIG_JSON" | jq \
            --arg port "$PORT_SS" \
            --arg psk "$PSK_SS" \
            '.inbounds += [{"type":"shadowsocks","listen":"::","listen_port":($port|tonumber),"method":"2022-blake3-aes-128-gcm","password":$psk,"tag":"ss-in"}]')
    fi
    ...
    if $INSTALL_HY2; then
        CONFIG_JSON=$(echo "$CONFIG_JSON" | jq \
            --arg port "$PORT_HY2" \
            --arg psk "$PSK_HY2" \
            '.inbounds += [{"type":"hysteria2","tag":"hy2-in","listen":"::","listen_port":($port|tonumber),"users":[{"password":$psk}],"tls":{"enabled":true,"alpn":["h3"],"certificate_path":"/etc/sing-box/certs/fullchain.pem","key_path":"/etc/sing-box/certs/privkey.pem"}}]')
    fi

    if $INSTALL_REALITY; then
        CONFIG_JSON=$(echo "$CONFIG_JSON" | jq \
            --arg port "$PORT_REALITY" \
            --arg uuid "$UUID" \
            --arg pk "$REALITY_PK" \
            --arg sid "$REALITY_SID" \
            '.inbounds += [{"type":"vless","tag":"vless-in","listen":"::","listen_port":($port|tonumber),"tcp_fast_open":true,"users":[{"uuid":$uuid,"flow":"xtls-rprx-vision"}],"tls":{"enabled":true,"server_name":"addons.mozilla.org","reality":{"enabled":true,"handshake":{"server":"addons.mozilla.org","server_port":443},"private_key":$pk,"short_id":[$sid]}}}]')
    fi

    if $INSTALL_VLESS_WS; then
        CONFIG_JSON=$(echo "$CONFIG_JSON" | jq \
            --arg port "$PORT_VLESS_WS" \
            --arg uuid "$UUID_VLESS_WS" \
            '.inbounds += [{"type":"vless","tag":"vless-ws-in","listen":"::","listen_port":($port|tonumber),"tcp_fast_open":true,"users":[{"uuid":$uuid}],"transport":{"type":"ws","path":"/"}}]')
    fi

    echo "$CONFIG_JSON" > "$CONFIG_PATH"
    echo "生成配置: $(echo "$CONFIG_JSON" | jq '.inbounds | length') 个入站"
