...
$INSTALL_VLESS_WS && echo "  ✅ VLESS WS (CDN回源)" || echo "  ❌ VLESS WS (CDN回源)"
$INSTALL_ARGO && echo "  ✅ Argo 隧道" || echo "  ❌ Argo 隧道"
echo ""

# 如果只选 Argo，不需要安装 sing-box
...
action_argo_temp() {
    ...
    if [ -n "$argo_url" ]; then
        ...
        echo ""
        echo "=== Argo 临时隧道节点 ==="
        echo "vless://${VLESS_WS_UUID}@${argo_url#https://}:443?encryption=none&type=ws&path=%2F&security=tls&sni=${argo_url#https://}#argo-temp${node_suffix}"
        echo ""
        echo "（重启后隧道地址会变，需重新获取节点链接）"
    else
        warn "隧道连接未就绪，日志内容:"
...
UNIT
    ...
    if systemctl is-active cloudflared-tunnel >/dev/null 2>&1; then
        ...
        node_suffix=$(cat /root/node_names.txt 2>/dev/null || true)
        echo "=== Argo 固定隧道节点 ==="
        echo "vless://${VLESS_WS_UUID}@${tunnel_domain}:443?encryption=none&type=ws&path=%2F&security=tls&sni=${tunnel_domain}#argo${node_suffix}"
        echo ""
